...
function scene:create( event )
	...
	local background = display.newImageRect(UI, "icon/bg.png", display.contentWidth, display.contentHeight)
	background.x, background.y = display.contentWidth/2, display.contentHeight/2

	local fishing = display.newImage(UI, "icon/gofishing.png")
	fishing.x, fishing.y = display.contentWidth*0.1, display.contentWidth*0.05

	local store = display.newImage(UI, "icon/gostore.png")
	store.x, store.y = display.contentWidth*0.1, display.contentWidth*0.1
	...
	function removeFish( )
		print("removeFish 시작")
		for i = 1, 8 do
			if(fishes[num] ~= nil) then
				print("remove 하는 중")
				fishes[num] = "none" -- 상태 초기화
	...
	local function gotoFishing( event )
		removeFish()
	...
	fishing:addEventListener("tap", gotoFishing)
	...
	for i = 1, 4 do
		...
		buttons[i].x, buttons[i].y = buttons[i].x + 90, buttons[i].y + 35 + (i * 212.5) -- 위치 조정
		UI:insert(buttons[i])
	end
	...
	function rotateOrder() -- 새 주문 로테이션
		...
		if(G.cream <= 8 and G.redBean <= 8) then -- 떡밥이 적다면 주문을 많이 받지 않음
			-- max(G.cream, G.redBean)
			fish_required = math.random(2, 4) 
		else
			fish_required = math.random(2, 8)
		end
	    	...
	    	if ( event.target.isFocus ) then
	    		if event.target.x < customers.x + 200 and event.target.x > customers.x - 200 -- 손님에게 드래그 하였다면 
					...
					if fish_served == fish_required then
						soundPlay( 5 )
						print("성공")
						G.money = G.money + 200 * fish_required
	...
	function burnCount( num, filling ) -- 각 붕어마다 시간이 인스턴스로 관리되어야 함
		...
		time = timer.performWithDelay(1000, flipCounter, flipSec[num])
	end
	...
	if G.start == 1 then
		sec = 300
		leftTime = display.newText(G.timeBar, "Time: ".. sec , display.contentWidth/2 + 545, display.contentHeight/10 - 65, G.fontLight, 40)
		leftTime:toFront()
		...
		local function Counter(event)
			...
			if (sec == 0) then
				display.remove(G.timeBar)
				soundPlay( 10 )
				print("시간 종료")
				timer.cancelAll() -- 원래 쓰던 것은 timer.cancel(event.source)
				if G.money >= 10000 then -- 클리어
					print("클리어 성공, G.money: "..G.money)
					G.index = 3
				else -- 실패
					print("실패")
					G.index = 2
				end
				display.remove(leftTime)
				removeFish()
				composer.removeScene("mainGame")
		...
		local timeCount = timer.performWithDelay(1000, Counter, sec)

	--타임바
	
		G.start = 0
		timeBar_bg = display.newImageRect(G.timeBar, "icon/timeBar_bg.png", 1040, 60)
		timeBar_bg.x, timeBar_bg.y = display.contentWidth/2 - 70, 40
		timeBar = display.newRect(G.timeBar, 0, 50, 960, 20)
		timeBar.x, timeBar.y = display.contentWidth/2 - 70, 40
		timeBar:setFillColor(1, 0.3, 0.5)

		local function loseTime (event)
			timeBar.x = timeBar.x - (sec/960/2) -- 타이머 바 고정
			timeBar.width = timeBar.width - (sec/960)
			if (sec == 0) then
				timer.cancelAll()
			end
			if (sec == 30) then
...
function scene:hide( event )
	...
	elseif phase == "did" then
		...
		composer.removeScene("mainGame") -- 여기
	end
end
