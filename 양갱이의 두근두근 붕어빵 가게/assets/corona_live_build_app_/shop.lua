...
function scene:create( event )
  ...
  local UI = display.newGroup()
	...
	local timeBack = display.newRoundedRect(UI, display.contentWidth/2 + 545, display.contentHeight/10 - 65, 220, 50, 55)
  timeBack:setFillColor(0)
  timeBack.alpha = 0.5
   
   local moneyback = display.newCircle(UI, display.contentWidth*0.9, display.contentHeight/8, 95)
   moneyback.path.radius = 120
   transition.to( moneyback.path, { time=2000, radius=100 } )
   moneyback:setFillColor(1,1,1,0.75)
   
  local showMoney = display.newText(UI, G.money, display.contentWidth*0.9, display.contentHeight/8, G.fontLight)
